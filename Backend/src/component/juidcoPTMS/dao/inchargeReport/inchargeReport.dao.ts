...
class InchargeReportDao {
  ...
  static async inchargeReport(req: Request) {
    const { incharge_id, date, month, year, from_date, to_date } = req.body;
    ...
    const twoWheelerTotal = data.reduce((sum, item) => sum + item?.total_two_wheeler_collection, 0)
    const fourWheelerTotal = data.reduce((sum, item) => sum + item?.total_four_wheeler_collection, 0)
    const total = data.reduce((sum, item) => sum + item?.total_amount, 0)

    const totalData = {
      twoWheelerTotal: twoWheelerTotal,
      fourWheelerTotal: fourWheelerTotal,
      total: total
    }

    return generateRes(totalData);
  }
