...
class ParkingAreaDao {
  ...
  async get(req: Request) {
    ...
    try {
      ...
      // ------------------  FILTER ------------------//
      const conditions: string[] = [`ulb_id = '${ulb_id}'`]; // always enforce ulb_id
      ...
      if (zip_code) {
        conditions.push(`zip_code ILIKE '${zip_code}'`);
      }

      if (station) {
        conditions.push(`station ILIKE '${station}'`);
      }
    ...
    `;
      ...
      const dataResult = await prisma.$queryRawUnsafe<any[]>(query, limit, offset);
  ...
  async getActiveOnly(req: Request) {
    try {
      const ulb_id = req?.body?.auth?.ulb_id || 2
      const data = await prisma.parking_area.findMany({
        where: { status: 1, ulb_id: ulb_id },
