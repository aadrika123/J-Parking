...
static createReceiptOut = async (req: Request) => {
  const { out_time, receipt_no, out_amount, payment_mode, scheduler_id } = req.body;
  ...
  const data = await prisma.receipts.update({
    ...
    data: {
      ...
      is_paid: true,
      payment_mode: payment_mode as string,
      scheduler_id: scheduler_id // Storing scheduler_id from payload
    },
    include: {
      area: {
        select: {
